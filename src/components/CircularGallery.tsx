...
class Media {
  ...
  onResize({
    ...
    if (mobile) {
      this.plane.scale.x =
        (this.viewport.width * (this.screen.width * 0.8)) / this.screen.width;
      this.plane.scale.y = this.plane.scale.x * (5 / 3);
    } else {
      this.plane.scale.y =
...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    const mobile = isMobile();
    const dragMultiplier = mobile
      ? this.scrollSpeed * 0.05
      : this.scrollSpeed * 0.025;

    const distance = (this.start - x) * dragMultiplier;
    this.scroll.target = (this.scroll.position ?? 0) + distance;

    this.touchVelocity = x - this.lastTouchX;
    this.lastTouchX = x;
  }
  ...
  onTouchUp() {
    ...
    if (isMobile() && Math.abs(this.touchVelocity) > 1) {
      this.scroll.target -= this.touchVelocity * 0.3;
    }
